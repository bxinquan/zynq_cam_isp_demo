...
#https://zhuanlan.zhihu.com/p/40626620  基于边缘检测和色差的CFA插值：Hibbard原理

import numpy as np

def _interpolate_G_on_R(G_left, G_right, G_up, G_down) :
    if abs(G_left-G_right) < abs(G_up-G_down) : 
        return (G_left+G_right)//2
    elif abs(G_left-G_right) > abs(G_up-G_down) :
...
def demosaic_hibbard(raw, bayer) :
    ...
    #calc all G
    for x in range(1, rgbpad.shape[0]-1) :
        for y in range(1, rgbpad.shape[1]-1) :
            fmt = (((x&1)<<1)|(y&1)) ^ bayer
            G = 0
            if (0 == fmt) :   #R
...
rgb = demosaic_hibbard(raw,0)
rgb.tofile('raw_512x768_rggb_hibbard.rgb')

#raw = np.fromfile('raw_512x768_grbg.bin', np.uint8).reshape(768,512)
#rgb = demosaic_hibbard(raw,1)
#rgb.tofile('raw_512x768_grbg_hibbard.rgb')

#raw = np.fromfile('raw_512x768_gbrg.bin', np.uint8).reshape(768,512)
#rgb = demosaic_hibbard(raw,2)
#rgb.tofile('raw_512x768_gbrg_hibbard.rgb')

#raw = np.fromfile('raw_512x768_bggr.bin', np.uint8).reshape(768,512)
#rgb = demosaic_hibbard(raw,3)
#rgb.tofile('raw_512x768_bggr_hibbard.rgb')
