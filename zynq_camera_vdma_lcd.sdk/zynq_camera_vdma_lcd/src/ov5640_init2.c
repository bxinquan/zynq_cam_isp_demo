...
#define  OV5640_ID    0x78   //OV5640鐨処D
...
static int iic_init(void)
{
	...
	XIic_Config *cfg_ptr = XIic_LookupConfig(XPAR_AXI_IIC_1_DEVICE_ID);
	if (!cfg_ptr) {
	...
	XIntc_Connect(sys_intr_inst(), XPAR_INTC_0_IIC_1_VEC_ID, (XInterruptHandler) XIic_InterruptHandler, &iic);
	XIntc_Enable(sys_intr_inst(), XPAR_INTC_0_IIC_1_VEC_ID);
#else
...
//	sccb_write_reg16(0x3008, 0x82);
	...
	//绯荤粺鏃堕挓鍒嗛 Bit[7:4]:绯荤粺鏃堕挓鍒嗛 input clock =24Mhz, PCLK = 96Mhz
	sccb_write_reg16(0x3035,0x11);
//	sccb_write_reg16(0x3036,0x3c); //PLL鍊嶉  //48M
//	sccb_write_reg16(0x3036, 0x5A); //PLL鍊嶉  //72M
	sccb_write_reg16(0x3036, 0x78); //PLL鍊嶉  //96M
	sccb_write_reg16(0x3037, 0x13); //PLL鍒嗛鎺у埗
	sccb_write_reg16(0x3108, 0x01); //绯荤粺鏍瑰垎棰戝櫒
	...
	sccb_write_reg16(0x3808, 0x05);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟楂�4浣�
	sccb_write_reg16(0x3809, 0x00);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380a, 0x03);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟楂�3浣�
	sccb_write_reg16(0x380b, 0xc0);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380c, 0x08);  //姘村钩鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380d, 0x00);  //姘村钩鎬诲儚绱犲ぇ灏忎綆8浣�
	sccb_write_reg16(0x380e, 0x04);  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380f, 0x00);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
...
//	sccb_write_reg16(0x3036, 0x5A); //PLL鍊嶉  //72M
	...
	sccb_write_reg16(0x3808, 0x07);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟楂�4浣�
	sccb_write_reg16(0x3809, 0x80);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380a, 0x04);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟楂�3浣�
	sccb_write_reg16(0x380b, 0x38);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380c, 0x09);  //姘村钩鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380d, 0xc4);  //姘村钩鎬诲儚绱犲ぇ灏忎綆8浣�
	sccb_write_reg16(0x380e, 0x05);  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380f, 0x00);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
	sccb_write_reg16(0x3810, 0x00);
	sccb_write_reg16(0x3811, 0x10);
	sccb_write_reg16(0x3812, 0x00);
...
//	sccb_write_reg16(0x3036, 0x5A); //PLL鍊嶉  //72M
	...
	sccb_write_reg16(0x3806, 0x07);
	sccb_write_reg16(0x3807, 0x9b);
	sccb_write_reg16(0x3808, 0x0a);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟楂�4浣�
	sccb_write_reg16(0x3809, 0x00);  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380a, 0x07);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟楂�3浣�
	sccb_write_reg16(0x380b, 0x80);  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟浣�8浣�
	sccb_write_reg16(0x380c, 0x0b);  //姘村钩鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380d, 0x1c);  //姘村钩鎬诲儚绱犲ぇ灏忎綆8浣�
	sccb_write_reg16(0x380e, 0x07);  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
	sccb_write_reg16(0x380f, 0xb0);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
	sccb_write_reg16(0x3810, 0x00);
	sccb_write_reg16(0x3811, 0x10);
	sccb_write_reg16(0x3812, 0x00);
	sccb_write_reg16(0x3813, 0x04);
...
int cmos_set_exposure2(unsigned exposure)
{
	...
	if (exposure < total_v_std) {
		sccb_write_reg16(0x380e, total_v_std>>8);  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
		sccb_write_reg16(0x380f, total_v_std&0xff);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
	} else {
		sccb_write_reg16(0x380e, exposure>>8);  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
		sccb_write_reg16(0x380f, exposure&0xff);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
	}
	return 0;
}
...
int ov5640_init2(u16 cmos_h_pixel,  u16 cmos_v_pixel, u16* ptr_total_v_std)
{
	...
	usleep(20000);  //OV5640涓婄數鍒板紑濮嬮厤缃畇ccb鑷冲皯绛夊緟20ms
    ...
    if(cam_id != 0x5640) { //鑾峰彇鍒版纭殑OV5640 ID
    	printf("cam_id Invalid %04X\r\n", cam_id);
    	return 1;
    }else{
		//鍏堝瀵勫瓨鍣ㄨ繘琛岃蒋浠跺浣嶏紝浣垮瘎瀛樺櫒鎭㈠鍒濆鍊�
		//瀵勫瓨鍣ㄨ蒋浠跺浣嶅悗锛岄渶瑕佸欢鏃�1ms鎵嶈兘閰嶇疆鍏跺畠瀵勫瓨鍣�
		sccb_write_reg16(0x3008,0x82); //Bit[7]:澶嶄綅 Bit[6]:鐢垫簮浼戠湢
...
#else
		sccb_write_reg16(0x3008,0x02); //姝ｅ父宸ヤ綔妯″紡
		sccb_write_reg16(0x3103,0x02); //Bit[1]:1 PLL Clock
		//寮曡剼杈撳叆/杈撳嚭鎺у埗 FREX/VSYNC/HREF/PCLK/D[9:6]
		sccb_write_reg16(0x3017,0xff);
		//寮曡剼杈撳叆/杈撳嚭鎺у埗 D[5:0]/GPIO1/GPIO0
		sccb_write_reg16(0x3018,0xff);
		sccb_write_reg16(0x3037,0x13); //PLL鍒嗛鎺у埗
		sccb_write_reg16(0x3108,0x01); //绯荤粺鏍瑰垎棰戝櫒
		sccb_write_reg16(0x3630,0x36);
		...
		sccb_write_reg16(0x3901,0x0a);
		sccb_write_reg16(0x3731,0x12);
		sccb_write_reg16(0x3600,0x08); //VCM鎺у埗,鐢ㄤ簬鑷姩鑱氱劍
		sccb_write_reg16(0x3601,0x33); //VCM鎺у埗,鐢ㄤ簬鑷姩鑱氱劍
		sccb_write_reg16(0x302d,0x60); //绯荤粺鎺у埗
		sccb_write_reg16(0x3620,0x52);
		sccb_write_reg16(0x371b,0x20);
		sccb_write_reg16(0x471c,0x50);
		sccb_write_reg16(0x3a13,0x43); //AEC(鑷姩鏇濆厜鎺у埗)
		sccb_write_reg16(0x3a18,0x00); //AEC 澧炵泭涓婇檺
		sccb_write_reg16(0x3a19,0xf8); //AEC 澧炵泭涓婇檺
		sccb_write_reg16(0x3635,0x13);
		sccb_write_reg16(0x3636,0x03);
		sccb_write_reg16(0x3634,0x40);
		sccb_write_reg16(0x3622,0x01);
		...
		sccb_write_reg16(0x3c04,0x28);
		sccb_write_reg16(0x3c05,0x98);
		sccb_write_reg16(0x3c06,0x00); //light meter 1 闃堝�糩15:8]
		sccb_write_reg16(0x3c07,0x08); //light meter 1 闃堝�糩7:0]
		sccb_write_reg16(0x3c08,0x00); //light meter 2 闃堝�糩15:8]
		sccb_write_reg16(0x3c09,0x1c); //light meter 2 闃堝�糩7:0]
		...
		sccb_write_reg16(0x3708,0x64);
		sccb_write_reg16(0x4001,0x02); //BLC(榛戠數骞虫牎鍑�)琛ュ伩璧峰琛屽彿
		sccb_write_reg16(0x4005,0x1a); //BLC(榛戠數骞虫牎鍑�)琛ュ伩濮嬬粓鏇存柊
		sccb_write_reg16(0x3000,0x00); //绯荤粺鍧楀浣嶆帶鍒�
		sccb_write_reg16(0x3004,0xff); //鏃堕挓浣胯兘鎺у埗
		sccb_write_reg16(0x4300,0xf8); //鏍煎紡鎺у埗 RAW
		sccb_write_reg16(0x501f,0x03); //ISP RAW
		sccb_write_reg16(0x440e,0x00);
		sccb_write_reg16(0x5000,0xa7); //ISP鎺у埗
		sccb_write_reg16(0x3a0f,0x30); //AEC鎺у埗;stable range in high
		sccb_write_reg16(0x3a10,0x28); //AEC鎺у埗;stable range in low
		sccb_write_reg16(0x3a1b,0x30); //AEC鎺у埗;stable range out high
		sccb_write_reg16(0x3a1e,0x26); //AEC鎺у埗;stable range out low
		sccb_write_reg16(0x3a11,0x60); //AEC鎺у埗; fast zone high
		sccb_write_reg16(0x3a1f,0x14); //AEC鎺у埗; fast zone low
		//LENC(闀滃ご鏍℃)鎺у埗 0x5800~0x583d
		...
		//SDE(鐗规畩鏁扮爜鏁堟灉)鎺у埗 0x5580~0x558b
		...
		//绯荤粺鏃堕挓鍒嗛 Bit[7:4]:绯荤粺鏃堕挓鍒嗛 input clock =24Mhz, PCLK = 72Mhz
		sccb_write_reg16(0x3035,0x11);
//		sccb_write_reg16(0x3036,0x3c); //PLL鍊嶉  //48M
		sccb_write_reg16(0x3036,0x46); //PLL鍊嶉  //72M
		sccb_write_reg16(0x3c07,0x08);
		//鏃跺簭鎺у埗 0x3800~0x3821
		sccb_write_reg16(0x3820,0x46);
		...
		//璁剧疆杈撳嚭鍍忕礌涓暟
		sccb_write_reg16(0x3808, cmos_h_pixel >> 8     );  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟楂�4浣�
		sccb_write_reg16(0x3809, cmos_h_pixel & 0x00FF );  //DVP 杈撳嚭姘村钩鍍忕礌鐐规暟浣�8浣�
		sccb_write_reg16(0x380a, cmos_v_pixel >> 8     );  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟楂�3浣�
		sccb_write_reg16(0x380b, cmos_v_pixel & 0x00FF );  //DVP 杈撳嚭鍨傜洿鍍忕礌鐐规暟浣�8浣�
		sccb_write_reg16(0x380c, total_h_pixel >> 8    );  //姘村钩鎬诲儚绱犲ぇ灏忛珮5浣�
		sccb_write_reg16(0x380d, total_h_pixel & 0x00FF);  //姘村钩鎬诲儚绱犲ぇ灏忎綆8浣�
		sccb_write_reg16(0x380e, total_v_pixel >> 8    );  //鍨傜洿鎬诲儚绱犲ぇ灏忛珮5浣�
		sccb_write_reg16(0x380f, total_v_pixel & 0x00FF);  //鍨傜洿鎬诲儚绱犲ぇ灏忎綆8浣�
		...
		sccb_write_reg16(0x4004,0x02); //BLC(鑳屽厜) 2 lines
		sccb_write_reg16(0x4713,0x03); //JPEG mode 3
		sccb_write_reg16(0x4407,0x04); //閲忓寲鏍囧害
		sccb_write_reg16(0x460c,0x22);
		sccb_write_reg16(0x4837,0x22); //DVP CLK divider
		sccb_write_reg16(0x3824,0x02); //DVP CLK divider
		sccb_write_reg16(0x5001,0x00); //ISP 鎺у埗
		sccb_write_reg16(0x3b07,0x0a); //甯ф洕鍏夋ā寮�
		//褰╂潯娴嬭瘯浣胯兘
		sccb_write_reg16(0x503d,0x00); //0x00:姝ｅ父妯″紡 0x80:褰╂潯鏄剧ず
		//娴嬭瘯闂厜鐏姛鑳�
		sccb_write_reg16(0x3016,0x02);
		sccb_write_reg16(0x301c,0x02);
		sccb_write_reg16(0x3019,0x02); //鎵撳紑闂厜鐏�
		sccb_write_reg16(0x3019,0x00); //鍏抽棴闂厜鐏�
#endif
		return 0;
	}
